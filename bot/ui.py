...
def run_ui() -> None:
    ...
    try:
        settings = AppSettings(
            ...
            backtest_years=int(raw.get("backtest_years", 3)),
    ...
    def _save_current_settings():
        ...
        try: settings.backtest_years = int(years_var.get())
        except Exception: pass
        try: settings.backtest_source = BacktestSource(bsrc_var.get())
        ...
        d = dict(
            ...
            extra_strategy_paths=settings.extra_strategy_paths, backtest_years=settings.backtest_years,
    ...
    ctk.CTkLabel(bt_config, text="Years of data:").grid(row=0, column=0, padx=10, pady=10, sticky="e")
    years_var = tk.StringVar(value=str(settings.backtest_years))
    ctk.CTkEntry(bt_config, textvariable=years_var, width=80).grid(row=0, column=1, padx=10, pady=10, sticky="w")
        
    ctk.CTkLabel(bt_config, text="Data source:").grid(row=1, column=0, padx=10, pady=10, sticky="e")
    bsrc_var = tk.StringVar(value=(settings.backtest_source.value if hasattr(settings.backtest_source,"value") else str(settings.backtest_source)))
    ctk.CTkOptionMenu(bt_config, variable=bsrc_var, values=["alpaca","yahoo","csv"]).grid(row=1, column=1, padx=10, pady=10, sticky="w")
        
    bt_results_header = ctk.CTkLabel(tab_backtest, text="Latest Backtest Results", font=("Arial", 16, "bold"))
    ...
    def update_backtest_results():
        ...
        if run and (run / "trades.csv").exists():
            ...
            try:
                ...
                bt_results_text.insert("1.0", f"{stats_text}Win Rate: {win_rate:.2f}%\nTotal P&L: ${total_pnl:.2f}\nAverage Win: ${avg_win:.2f}\nAverage Loss: ${avg_loss:.2f}\n")
                bt_results_text.configure(state="disabled")
            except Exception as e:
                logging.error(f"Failed to load backtest results: {e}")
    root.after(10000, update_backtest_results)
